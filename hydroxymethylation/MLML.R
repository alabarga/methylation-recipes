MLML <- function (G.matrix = NULL, H.matrix = NULL, L.matrix = NULL, 
          M.matrix = NULL, T.matrix = NULL, U.matrix = NULL, iterative = FALSE, 
          tol = 1e-05) 
{
  ...
  four <- function(x1, x2, x3, x4) {
    return(identical(x1, x2) & identical(x1, x3) & identical(x1, 
                                                             x4))
  }
  six <- function(x1, x2, x3, x4, x5, x6) {
    return(identical(x1, x2) & identical(x1, x3) & identical(x1, 
                                                             x4) & identical(x1, x5) & identical(x1, x6))
  }
  pme <- 0.3
  phe <- 0.5
  diff <- 1
  pm <- matrix()
  ph <- matrix()
  if (!is.null(G.matrix) & !is.null(H.matrix) & !is.null(L.matrix) & 
      !is.null(M.matrix) & !is.null(T.matrix) & !is.null(U.matrix)) {
    if (!six(rownames(L.matrix), rownames(M.matrix), rownames(T.matrix), 
      ...
      if (!iterative) {
        ...
        pm0 <- pm <- ifelse((m == 0 | h == 0 | u == 0 | 
                               g == 0 | l == 0 | t == 0) & (m/(m + l) + h/(h + 
                                                                             g) + u/(u + t)) >= 1, pm0/ss, pm0)
        ph0 <- ph <- ifelse((m == 0 | h == 0 | u == 0 | 
                               g == 0 | l == 0 | t == 0) & (m/(m + l) + h/(h + 
                                                                             g) + u/(u + t)) >= 1, ph0/ss, ph0)
        pc0 <- pc <- ifelse((m == 0 | h == 0 | u == 0 | 
                               g == 0 | l == 0 | t == 0) & (m/(m + l) + h/(h + 
                                                                             g) + u/(u + t)) >= 1, pc0/ss, pc0)
        Vm <- pm * (1 - pm)/(m + l)
        Vh <- ph * (1 - ph)/(h + g)
        Vc <- pc * (1 - pc)/(u + t)
        ...
        while (diff > tol) {
          ...
          phe <- ((h - k + t)/(h + g + t + u - j - k)) * 
            (1 - pme)
          diff_pme <- abs(max(pme - pme_ant))
          diff_phe <- abs(max(phe[!is.na(phe)] - phe_ant))
    ...
    if (!four(rownames(G.matrix), rownames(H.matrix), rownames(M.matrix), 
      ...
      if (!iterative) {
        pme = ifelse(m/(m + l) <= g/(h + g), m/(m + l), 
                     (g + m)/(g + h + m + l))
        phe = ifelse(m/(m + l) <= g/(h + g), h/(h + g), 
                     (h + l)/(g + h + m + l))
      }
      else {
        print("4")
